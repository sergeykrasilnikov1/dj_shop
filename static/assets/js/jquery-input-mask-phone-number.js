...
(function ($) {
    $.fn.usPhoneFormat = function (options) {
        ...
        } else if (params.format === 'xx(xxx) xxx-xxxx') {
            $(this).on('keydown touchend', function (e) {
                ...
                if (curchr == 5 && e.which != 8 && e.which != 0) {
                    $(this).val(curval.slice(0,2) + '(' + curval.slice(2,5) + ')' + " ");
            ...
            $(this).bind('paste', function (e) {
                ...
                if (!$.isNumeric(inputValue)) {
                    ...
                    if (inputValue.length > 11) {
                        inputValue = String(inputValue.replace(/(\d{2})(\d{3})(\d{3})(\d{4})/, "$1($2) $3-$4"));
                    } else if (inputValue.length > 8) {
                        inputValue = String(inputValue.replace(/(\d{2})(\d{3})(\d{3})(?=\d)/g, '$1($2) $3-'));
                    } else if (inputValue.length > 5) {
                        inputValue = String(inputValue.replace(/(\d{2})(\d{3})(?=\d)/g, '$1($2) '));
                    } else if (inputValue.length > 2) {
                        inputValue = String(inputValue.replace(/(\d{2})(?=\d)/g, '$1 '));
                    }
